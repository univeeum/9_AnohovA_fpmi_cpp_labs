...
// MATRIX FUNCTIONS
void AllocateMatrix(double**& matrix, double*& x, double*& y, const size_t n) {
    x = new double[n];
    y = new double[n];
    matrix = new double*[n];
...
void DeleteMatrix(double**& matrix, double* x, double* y, const size_t n) {
    for (size_t i = 0; i < n; i++) {
        delete[] matrix[i];
    }
    delete[] matrix;
...
void PrintMatrix(double** matrix, const size_t n) {
    std::cout << "Matrix:\n";
...
void FillMatrix(double**& matrix, double* x, double* y, size_t size) {
...
void PrintColumnSums(double** matrix, const size_t n) {
...
void PrintArrays(double* x, double* y, const size_t size) {
...
// INPUT MATRIX
void RandomInput(double* x, double* y, size_t size) {
    int32_t ra, rb;
...
void ManualInput(double* x, double* y, size_t size) {
...
void InputMatrix(double**& matrix, double* x, double* y, size_t size) {
